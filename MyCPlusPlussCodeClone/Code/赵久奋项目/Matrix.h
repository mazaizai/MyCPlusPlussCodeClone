...
public:
	...
	void ShowData();
	CMatrix L_(char axix, double rad);   //基元变换矩阵    axis: 轴 1:x ,2:y ,3:z    angle  角度  单位 度
	...
	//运算函数
	CMatrix X_(double x0, double y0, double z0);

	//运算符重载
	  double operator () (unsigned int row,unsigned int col);  //提取某一矩阵元素
